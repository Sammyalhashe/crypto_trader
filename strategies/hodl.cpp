...
HodlStrategy::~HodlStrategy() {
    d_positionManager.unregister_observer(this);
}
...
void HodlStrategy::on_position_update(const std::string& symbol, double new_position) {
...
void HodlStrategy::goOverTradesAtPrice(const std::string_view& product_sv,
    ...
    if (!state.waitingForSell) {
        if (d_config.initStrategy() == HodlStrategyConfig::e_BUY_IMMEDIATELY) {
            ...
            // e_SET_BASIS_PRICE
            if (state.lastBuyPrice == 0.0) {
                 state.lastBuyPrice = price;
            } else if (price <= computeXPercentDown(state.lastBuyPrice, d_config.percentDown())) {
                buy(product, price, std::string(timestamp));
            }
        }
    } else {
        auto currentPosition = d_positionManager.currentHoldings(product).value_or(0.0);
        if (currentPosition > 0) {
            if (price >= computeXPercentUp(state.lastBuyPrice, d_config.percentUp())) {
                sell(product, price, std::string(timestamp));
            } else if (price <= computeXPercentDown(state.lastBuyPrice, d_config.percentDown()) && !state.hasBoughtAgain) {
                buy(product, price, std::string(timestamp));
...
void HodlStrategy::buy(const std::string& product, double price, const std::string& timestamp) {
    SPDLOG_INFO("HODL: BUY {} at price {}", product, price);
    if (d_config.emit()) {
        common::Action action{
            .d_type = common::Side::e_BUY,
            .d_product = product,
            .d_quantity = convertFromUSDToProductAmount(price, d_config.buyAmount())
        };
        d_config.emit()(action);
    }
}

void HodlStrategy::sell(const std::string& product, double price, const std::string& timestamp) {
    SPDLOG_INFO("HODL: SELL {} at price {}", product, price);
    auto currentPosition = d_positionManager.currentHoldings(product).value_or(0.0);
    if (d_config.emit() && currentPosition > 0) {
        common::Action action{
