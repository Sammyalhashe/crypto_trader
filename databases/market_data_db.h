...
public:
    ...
    // MANIPULATORS
    inline void add_data(const std::string& symbol, const MarketDataType& market_data) noexcept;
...
// PUBLIC MANIPULATORS 
template<class MarketDataType>
inline void MarketDataDB<MarketDataType>::add_data(const std::string& symbol, const MarketDataType& market_data) noexcept {
...
template<class Archive>
void MarketDataDB<MarketData>::serialize(Archive& archive, const unsigned int version) {
